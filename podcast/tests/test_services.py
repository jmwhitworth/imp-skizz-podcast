...
class services__podcast_create_from_youtubevideo__TestCase(TestCase):
    def test_creates_podcast_when_episode_identified(self, mock_llm_client):
        mock_llm_client.return_value.identify_episode.return_value = EpisodeIdentityData(
            reasoning="test", is_episode=True, season=2, episode=7
        )
    ...
    def test_archives_video_when_podcast_already_exists_for_season_episode(
        ...
        mock_llm_client.return_value.identify_episode.return_value = EpisodeIdentityData(
            reasoning="test", is_episode=True, season=1, episode=3
        )
...
@patch("podcast.services.LLMClient")
class services__podcast_create_from_youtubevideos__TestCase(TestCase):
    def test_creates_podcasts_for_unlinked_non_archived_videos(self, mock_llm_client):
        ...
        created_count = podcast_create_from_youtubevideos()
...
class services__podcast_update_title__TestCase(TestCase):
    def test_returns_false_when_podcast_not_found(self):
        self.assertFalse(podcast_update_title(uuid.uuid4()))
    ...
    def test_updates_title_when_renamed(self, mock_llm_client):
        ...
        result = podcast_update_title(podcast.id)
...
@patch("podcast.services.LLMClient")
class services__youtubevideo_create_podcast__TestCase(TestCase):
    def test_returns_none_when_video_not_found(self, mock_llm_client):
        self.assertIsNone(youtubevideo_create_podcast(uuid.uuid4()))
        mock_llm_client.return_value.identify_episode.assert_not_called()

    def test_creates_podcast_for_existing_video(self, mock_llm_client):
        mock_llm_client.return_value.identify_episode.return_value = EpisodeIdentityData(
            reasoning="", is_episode=True, season=1, episode=9
        )
        mock_llm_client.return_value.rename_episode.side_effect = lambda title: title
        video = YoutubeVideoFactory()

        podcast = youtubevideo_create_podcast(video.id)
...
class services__youtubevideo_import__TestCase(TestCase):
    ...
    def test_imports_videos_and_creates_podcasts(
        ...
        mock_llm_client.return_value.identify_episode.return_value = EpisodeIdentityData(
            reasoning="", is_episode=True, season=1, episode=42
        )
        mock_llm_client.return_value.rename_episode.side_effect = lambda title: title
        mock_youtube_client.return_value.fetch_recent_uploads.return_value = [
            RemoteVideo(
...
@patch("podcast.services.spotifyepisode_assign_podcast")
...
class services__spotifyepisode_assign_podcast__TestCase(TestCase):
    def test_returns_none_when_episode_not_found(self):
        self.assertIsNone(spotifyepisode_assign_podcast(uuid.uuid4()))
    ...
    def test_matches_unique_release_date(self):
        ...
        result = spotifyepisode_assign_podcast(episode.id)
    ...
    def test_falls_back_to_season_episode_when_release_date_is_ambiguous(
        ...
        mock_llm_client.return_value.identify_episode.return_value = EpisodeIdentityData(
            reasoning="", is_episode=True, season=1, episode=2
        )
